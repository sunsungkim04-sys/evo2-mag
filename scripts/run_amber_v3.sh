...
OUTPUT_DIR="$RESULTS_DIR/amber_eval_v3"
mkdir -p "$OUTPUT_DIR"
...
for i in $(seq 0 20); do
    ...
    # Enhanced v3
    enhanced="$OUTPUT_DIR/enhanced_v3_${i}.tsv"
    echo -e "@Version:0.9.1\n@SampleID:${sample}\n@@SEQUENCEID\tBINID" > "$enhanced"
    bindir_v3="$RESULTS_DIR/${sample}/tmp/binning/round_1/binette_enhanced_v3/final_bins"
    if [ -d "$bindir_v3" ]; then
        for f in "$bindir_v3"/*.fa; do
            [ -f "$f" ] || continue
            bn="enh3_$(basename "$f" .fa)"
            grep "^>" "$f" | sed "s/^>//" | while read c; do echo -e "${c}\t${bn}"; done
        done >> "$enhanced"
    fi
done
...
# Combine
for prefix in gold baseline enhanced_v3; do
...
~/miniforge3/bin/amber.py \
    ...
    "$OUTPUT_DIR/enhanced_v3_combined.tsv" \
    -l "Baseline,Enhanced_v3" \
