#!/bin/bash
# Binette 앙상블: 기존 3종(VAMB + MetaBAT2 + SemiBin2) + Evo2 v4_1 bins = 4종
# v4_1 = v4 (임베딩+커버리지) + cosine metric
# Usage: bash ~/evo2-mag/scripts/run_binette_enhanced_v4_1.sh
...
echo "=== [$(date)] Binette Enhanced v4_1 앙상블 시작 ==="
echo "  4종: VAMB + MetaBAT2 + SemiBin2 + Evo2_v4_1"
...
run_sample() {
    ...
    local evo2dir="${RESULTS_DIR}/evo2_bins_v4_1/${sample}"
    local outdir="${bindir}/binette_enhanced_v4_1"
    local contigs="${bindir}/contigs.fasta"
    local tmpdir="${bindir}/tmp_decompress_v4_1"
    ...
    if [ ! -d "$evo2dir" ] || [ -z "$(ls -A "$evo2dir" 2>/dev/null)" ]; then
        echo "  [sample_${i}] Evo2 v4_1 bins 없음, 스킵"
        return
    fi
    ...
    echo "  [$(date +%H:%M:%S)] sample_${i} 시작 (vamb=$vamb_n mb2=$mb2_n sb2=$sb2_n evo2_v4_1=$evo2_n)"
    ...
    # Evo2 v4_1 bins 필터링: contigs.fasta에 없는 contig 제거
...
" 2>/dev/null
    ...
    echo "    evo2_v4_1 filtered: $evo2_n → $evo2_filtered_n bins (contigs.fasta 기준)"
    ...
    echo "  [$(date +%H:%M:%S)] sample_${i} 완료 → ${result_n} enhanced_v4_1 bins"
}
...
echo "--- Enhanced v4_1 Binette 결과 요약 ---"
total_baseline=0
total_v1=0
total_v4_1=0
for i in $(seq 0 20); do
    outdir_v4_1="${RESULTS_DIR}/baseline_sample${i}/tmp/binning/round_1/binette_enhanced_v4_1"
    ...
    n_v4_1=0; n_v1=0; n_base=0
    [ -d "$outdir_v4_1/final_bins" ] && n_v4_1=$(ls "$outdir_v4_1/final_bins/"*.fa 2>/dev/null | wc -l)
    ...
    total_v4_1=$((total_v4_1 + n_v4_1))
    echo "  sample_${i}: baseline=${n_base} → v1=${n_v1} → v4_1=${n_v4_1}"
done
echo ""
echo "  TOTAL: baseline=${total_baseline} → v1=${total_v1} → v4_1=${total_v4_1}"
