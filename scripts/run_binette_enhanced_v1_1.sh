#!/bin/bash
# Binette 앙상블: 기존 3종(VAMB + MetaBAT2 + SemiBin2) + Evo2 v1_1 bins (cosine) = 4종
# Usage: bash ~/evo2-mag/scripts/run_binette_enhanced_v1_1.sh
...
echo "=== [$(date)] Binette Enhanced v1_1 (cosine) 앙상블 시작 ==="
echo "  4종: VAMB + MetaBAT2 + SemiBin2 + Evo2(cosine)"
...
run_sample() {
    ...
    local evo2dir="${RESULTS_DIR}/evo2_bins_v1_1/${sample}"
    local outdir="${bindir}/binette_enhanced_v1_1"
    local contigs="${bindir}/contigs.fasta"
    local tmpdir="${bindir}/tmp_decompress_v1_1"
    ...
    if [ ! -d "$evo2dir" ] || [ -z "$(ls -A "$evo2dir" 2>/dev/null)" ]; then
        echo "  [sample_${i}] Evo2 v1_1 bins 없음, 스킵"
        return
    fi
...
echo "--- Enhanced v1_1 Binette 결과 요약 ---"
for i in $(seq 0 20); do
    outdir="${RESULTS_DIR}/baseline_sample${i}/tmp/binning/round_1/binette_enhanced_v1_1"
    if [ -d "$outdir/final_bins" ]; then
        ...
        echo "  sample_${i}: baseline=${orig_total} → enhanced_v1_1=${n}"
    else
