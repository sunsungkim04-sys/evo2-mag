...
- **Phase 2** 🔄 PC101에서 CAMI2 baseline 실행 중
- **Phase 3** ⏳ Evo 2 추론 — H100 SXM (7B 모델)
- **Phase 4** ⏳ AMBER 평가
...
2. **비닝** (`bin.py`): HDBSCAN(min_cluster_size=5) → evo2_c2b.tsv → Binette 5번째 binner
3. **키메라** (`chimera.py`): 슬라이딩 윈도우(10kb, 5kb step) perplexity → 2σ 이상 구간 = 오염 의심
4. **Binette 재실행**: MetaBAT2 + SemiBin2 + VAMB + GraphMB + Evo2 = 5개 합의

## A/B 비교 설계

| 태스크 | Baseline A | Enhanced B | 지표 |
|--------|-----------|-----------|------|
| 비닝 | MetaBAT2+SemiBin2+VAMB+GraphMB → Binette | +Evo2 임베딩 (5번째) → Binette | HQ/MQ MAG 수, ARI |
| 키메라 탐지 | CheckM2 | CheckM2+Evo2 perplexity | 오염률 |
| 기능 주석 | Prokka/eggNOG | +Evo2 likelihood | hypothetical → 기능 비율 |

## Phase 3 실행 환경 (GPU)

- 7B 모델: **H100 SXM × 1** ← 현재 사용 중
