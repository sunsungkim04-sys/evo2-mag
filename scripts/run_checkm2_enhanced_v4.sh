#!/bin/bash
# CheckM2 품질 평가: Enhanced v4 Binette bins
# Usage: bash ~/evo2-mag/scripts/run_checkm2_enhanced_v4.sh
...
THREADS=72
OUTPUT="$RESULTS_DIR/checkm2_enhanced_v4"

echo "=== [$(date)] CheckM2 Enhanced v4 bins 품질 평가 시작 ==="

# 1) 모든 enhanced v2 bins를 한 디렉토리에 모으기
ALLBINS="$OUTPUT/all_bins"
mkdir -p "$ALLBINS"

total=0
for i in $(seq 0 20); do
    bindir="$RESULTS_DIR/baseline_sample${i}/tmp/binning/round_1/binette_enhanced_v4/final_bins"
    if [ -d "$bindir" ]; then
...
echo "  $total enhanced_v4 bins 수집 완료"
...
if [ -f "$REPORT" ]; then
    ...
    echo ""
    echo "--- Enhanced v4 CheckM2 결과 요약 ---"
  ...
  Enhanced v2:  HQ 45, MQ 94 (total 165)"
    echo "  Enhanced v4:  HQ $hq, MQ $mq (total $total)"
    echo ""
    echo "  상세: $REPORT"
fi
