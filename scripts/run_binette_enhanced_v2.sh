...
# v1 대비 변경: evo2_bins → evo2_bins_v2 (샘플별 UMAP+HDBSCAN)
# Usage: bash ~/evo2-mag/scripts/run_binette_enhanced_v2.sh
...
echo "=== [$(date)] Binette Enhanced v2 앙상블 시작 ==="
echo "  4종: VAMB + MetaBAT2 + SemiBin2 + Evo2_v2"
...
run_sample() {
    ...
    local evo2dir="${RESULTS_DIR}/evo2_bins_v2/${sample}"
    local outdir="${bindir}/binette_enhanced_v2"
    local contigs="${bindir}/contigs.fasta"
    local tmpdir="${bindir}/tmp_decompress_v2"
    ...
    echo "  [$(date +%H:%M:%S)] sample_${i} 시작 (vamb=$vamb_n mb2=$mb2_n sb2=$sb2_n evo2_v2=$evo2_n)"
...
" 2>/dev/null
    ...
    echo "    evo2_v2 filtered: $evo2_n → $evo2_filtered_n bins (contigs.fasta 기준)"
    ...
    echo "  [$(date +%H:%M:%S)] sample_${i} 완료 → ${result_n} enhanced_v2 bins"
}
...
echo "--- Enhanced v2 Binette 결과 요약 ---"
total_baseline=0
total_v1=0
total_v2=0
for i in $(seq 0 20); do
    outdir_v2="${RESULTS_DIR}/baseline_sample${i}/tmp/binning/round_1/binette_enhanced_v2"
