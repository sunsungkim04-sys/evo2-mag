#!/bin/bash
# CheckM2 품질 평가: Enhanced v1_1 (cosine) Binette bins
# Usage: bash ~/evo2-mag/scripts/run_checkm2_enhanced_v1_1.sh
...
THREADS=72
OUTPUT="$RESULTS_DIR/checkm2_enhanced_v1_1"

echo "=== [$(date)] CheckM2 Enhanced v1_1 (cosine) 품질 평가 시작 ==="

# 1) 모든 enhanced v1_1 bins를 한 디렉토리에 모으기
ALLBINS="$OUTPUT/all_bins"
mkdir -p "$ALLBINS"

total=0
for i in $(seq 0 20); do
    bindir="$RESULTS_DIR/baseline_sample${i}/tmp/binning/round_1/binette_enhanced_v1_1/final_bins"
    if [ -d "$bindir" ]; then
...
echo "  $total enhanced v1_1 bins 수집 완료"
...
if [ -f "$REPORT" ]; then
    ...
    echo ""
    echo "--- Enhanced v1_1 (cosine) CheckM2 결과 요약 ---"
    ...
    echo "  Enhanced v1_1: HQ $hq, MQ $mq (total $total)"
    echo ""
    echo "  상세: $REPORT"
fi
