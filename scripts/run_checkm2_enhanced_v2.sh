#!/bin/bash
# CheckM2 품질 평가: Enhanced v2 Binette bins
# Usage: bash ~/evo2-mag/scripts/run_checkm2_enhanced_v2.sh
...
THREADS=72
OUTPUT="$RESULTS_DIR/checkm2_enhanced_v2"

echo "=== [$(date)] CheckM2 Enhanced v2 bins 품질 평가 시작 ==="
...
for i in $(seq 0 20); do
    bindir="$RESULTS_DIR/baseline_sample${i}/tmp/binning/round_1/binette_enhanced_v2/final_bins"
    if [ -d "$bindir" ]; then
...
echo "  $total enhanced_v2 bins 수집 완료"
...
if [ -f "$REPORT" ]; then
    ...
    echo ""
    echo "--- Enhanced v2 CheckM2 결과 요약 ---"
    ...
    echo "  Enhanced v2:  HQ $hq, MQ $mq (total $total)"
    echo ""
    echo "  상세: $REPORT"
fi
