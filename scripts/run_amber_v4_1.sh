#!/bin/bash
# AMBER 평가: Baseline vs Enhanced v4_1
set -eo pipefail

RESULTS_DIR="$HOME/results"
OUTPUT_DIR="$RESULTS_DIR/amber_eval_v4_1"
mkdir -p "$OUTPUT_DIR"

echo "=== [$(date)] AMBER v4_1 평가 시작 ==="
...
for i in $(seq 0 20); do
    ...
    # Enhanced v4_1
    enhanced="$OUTPUT_DIR/enhanced_v4_1_${i}.tsv"
    echo -e "@Version:0.9.1\n@SampleID:${sample}\n@@SEQUENCEID\tBINID" > "$enhanced"
    bindir_enh="$RESULTS_DIR/${sample}/tmp/binning/round_1/binette_enhanced_v4_1/final_bins"
    if [ -d "$bindir_enh" ]; then
        for f in "$bindir_enh"/*.fa; do
            [ -f "$f" ] || continue
            bn="enh4_1_$(basename "$f" .fa)"
            grep "^>" "$f" | sed "s/^>//" | while read c; do echo -e "${c}\t${bn}"; done
        done >> "$enhanced"
    fi
done
...
# Combine
for prefix in gold baseline enhanced_v4_1; do
...
~/miniforge3/bin/amber.py \
    ...
    "$OUTPUT_DIR/enhanced_v4_1_combined.tsv" \
    -l "Baseline,Enhanced_v4_1" \
    -o "$OUTPUT_DIR/amber_output" \
    2>&1

echo "=== [$(date)] AMBER v4_1 완료 ==="
